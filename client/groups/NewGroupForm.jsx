...
export default class NewGroupForm extends TrackerReact(React.Component) {
  constructor(){
    super();
  ...
  check(){
		const hasName = (this.refs.name.value != "");
		const requiresLeader = this.doesTypeRequireLeader();

		if ( requiresLeader ) {
			if(!hasName || this.state.leader.length==0){
	      ...
	      this.setState({createdisabled: false});
			...
			!hasName ? this.setState({createdisabled: true}) : this.setState({createdisabled: false});
		}
  }
  ...
  getUsers(){
	...
	doesTypeRequireLeader(){
		return (this.props.type=="Small Group"||this.props.type=="Team");
	}
