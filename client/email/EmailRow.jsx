...
export default class EmailSummary extends TrackerReact(React.Component){
  ...
  render() {
    ...
    <li className="collection-item trunc-cell">
      ...
      <span style={{width: "20%", display: "inline-block"}}>When</span>
    </li>
  )
    /*return (
    <tr className="trunc-cell">
      <td style={{width: "30%"}}>{email.subject}</td>
      <td style={{width: "20%"}}>To 1</td>
      <td style={{width: "10%"}}>{email.from}</td>
      <td style={{width: "20%"}}>{this.getTemplate()}</td>
