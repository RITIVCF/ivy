...
export default class Contact {
	...
	getFunnelStatus(){
		let record = null;
		if(Meteor.isClient){
			record = Funnel.find({_id: this._id}, {limit: 1, sort: {date: -1}}).fetch()[0];
		}
		else {
			//is server
			record = Funnel.find({uid: this._id}, {limit: 1, sort: {date: -1}}).fetch()[0];
		}
	...
	getStatus(){
		let record = null;
		if(Meteor.isClient){
			record = Status.find({_id: this._id}, {limit: 1, sort: {date: -1}}).fetch()[0];
		}
		else {
			//is server
			record = Status.find({uid: this._id}, {limit: 1, sort: {date: -1}}).fetch()[0];
		}
		if(record){
			return record.status;
		}
	...
	get isPrayerGroupLeader() {
		return !!Groups.findOne({_id: "prayergroupleaders", users: this._id});
	}
