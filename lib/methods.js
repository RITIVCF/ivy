...
Meteor.methods({
  ...
  createJobRequest(eid, uid, jobname, submitby, isreq){
    ...
    if(isreq){
      ...
      Email.send({
        ...
        html: "<p>Dear "+ user.name + "</p><p>You have been requested to serve in the position of "
        + jobname + " for the event <b>" + event.name + "</b> on <b>" + moment(event.start).format("Do MMM YY") + "</b> at <b>"
  ...
  addEventRequest(subj, desc, assg, assu, cust, eid, evreqtyp){
    ...
    var typs = Options.findOne("requesttypes").vals.filter((val)=>{
      return val.type==evreqtyp;
    });
    var groupid = typs[0].gid;
    //
    Tickets.insert({
      ticketnum: ret.seq,
      subject: subj,
