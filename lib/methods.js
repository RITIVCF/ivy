Meteor.methods({
  ...
  removeGroupFromPage(pageid, gid){
    if(this.props.group.name=="Administrator"){
      return;
    }
  ...
  // Attendance stuff
  createAttendanceRecord(eid,cid,first,tktId){
    Events.update(eid,{$addToSet: {"attendees":{"_id":cid,"firsttime":first, "ticket": tktId}}});
  },
