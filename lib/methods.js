...
Meteor.methods({
  ...
  updateEventEnd(eid,datetime){
	...
	updateEventImage(eid, url){
		Events.update(eid,{$set: {"pic": url}});
	},
  ...
  submitFeedback(subj, txt, typ){
    ...
    return Tickets.insert({
      ...
      lastUpdated: new Date()
		...
		console.log("Testing if is server");
		if(Meteor.isServer){
			console.log("Is server");
			newEmailJob({
				to: ["awe6013@rit.edu","weeksseth@gmail.com"],
				from: "Ivy Information System <ivcf@rit.edu>",
				Subject: "New Feedback: " + subj,
				html: "Type: " + typ + "<br>" + txt
			});
		}
  },
