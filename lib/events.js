...
checkEventPermission = function(ev){
	let permObject = {view: false, edit: false, owner: false};
	const isAdmin = Groups.find({_id:"admin", users: Meteor.userId()}).fetch().length==1;
	const isOwner = ev.owner==Meteor.userId();

	if ( isAdmin || isOwner ) {
		permObject.view = true;
		permObject.edit = true;
		permObject.owner = true;
		return permObject;
	}
  ...
  var perm = false;// ev.perm[""]
	...
	permObject.view = view;
	permObject.edit = perm;
  return permObject;
}
