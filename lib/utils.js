...
dateFormat = function(date, mask = "ddd MMM Do @ h:mm a"){
	return new moment(date.toISOString()).format(mask);
}
...
sendEmail = function ( emailObj ){
	...
	const overrideEmails = Options.findOne("emailsendoverride").val;
	if(overrideEmails){
		emailObj.to = ["awe6013@rit.edu","weeksseth@gmail.com"];
	}
